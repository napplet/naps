...
Result messages MAY include `error` when the request cannot be fulfilled. Common errors include `"unsupported rail"`, `"no server configured"`, `"file too large"`, `"unsupported media type"`, `"user cancelled"`, `"server rejected"`, `"upload failed"`, and `"quota exceeded"`.
